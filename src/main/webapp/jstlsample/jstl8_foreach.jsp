...
<%-- 	<c:forEach begin="1" end="10" var="i" varStatus="st">
		${i }
		<c:if test="${not st.last }">,</c:if> <!--'last'가 아니면 true,아니라면  ','콤마 찍어줌  -->
	</c:forEach> --%>
	
	<c:set value="0" var="n"/>
	<c:forEach begin="1" end="10" var="i" varStatus="st">
		${i }
		<c:set value="${n+i }" var="n"/> <!-- 'n' 값이 돌면서 'i'가 더해진다  -->
		<c:if test="${not st.last }">,</c:if> <!--'last'가 아니면 true,아니라면  ','콤마 찍어줌  -->
	</c:forEach>
	sum=${n }
	...
	<hr>
		...
		<c:set value="0" var="n" scope="application"/> <!--'application'으로 'n'값이 죽지않고 그대로 감-->
	<c:forEach begin="1" end="10" var="i" varStatus="st">
		${i }
		<c:set value="${n+i }" var="n"/> <!-- 'n' 값이 돌면서 'i'가 더해진다  -->
		<c:if test="${not st.last }">,</c:if> <!--'last'가 아니면 true,아니라면  ','콤마 찍어줌  -->
